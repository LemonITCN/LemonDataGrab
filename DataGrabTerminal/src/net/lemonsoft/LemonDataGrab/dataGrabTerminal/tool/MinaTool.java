...
import net.lemonsoft.LemonDataGrab.dataGrabTerminal.view.stage.MainStage;
...
public class MinaTool {
    ...
    public void connectToServer(String sessionFingerprint) {
        if (!isConnecting) {
            ...
            connector.getFilterChain().addLast("codec", new ProtocolCodecFilter(new TextLineCodecFactory(Charset.forName("UTF-8"), LineDelimiter.MAC.getValue(), LineDelimiter.MAC.getValue())));
                ...
                switch (info) {
                    case SESSION_BIND_SUCCESS:// 收到登录会话绑定成功的通知
                        Platform.runLater(new Runnable() {
                            @Override
                            public void run() {
                                LoginStage.sharedInstance().onLoginSuccess();
                            }
                        });
                        break;
                    case SESSION_SEND_TASK:// 收到一条任务推送
                        try {
                            ...
                            MainStage.sharedInstance().addTask(taskEntity);
                        }catch (Exception e){
                            e.printStackTrace();
                        }
                        break;
