    ...
    private class MinaToolHandler extends IoHandlerAdapter {
        ...
        public void sessionOpened(IoSession session) throws Exception {
            ...
            if (currentSessionFingerprint == null || currentSessionFingerprint.equals("")) {
                ...
                sendSuccessToServer(SESSION_SIGN_UP, new String[]{"isDataGrabTerminal", "sessionFingerprint"}, new Object[]{true, currentSessionFingerprint});
            }
        }
            ...
            if (success) {
                switch (info) {
                    case SESSION_BIND_SUCCESS:// 收到登录会话绑定成功的通知
                        ...
                        break;
                    case SESSION_SEND_TASK:// 收到一条任务推送
                        try {
                            TaskEntity taskEntity = new Gson().fromJson(new Gson().toJson(data.get("task")), TaskEntity.class);
                            Main.commandMain.defaultMainViewController.addTask(taskEntity);
                            TaskService.executeNextTask();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        break;
                    case SESSION_QUERY_LOCAL_TASK_LIST:// 需要查询本地的已经采集的所有任务列表
                        TaskService.uploadLocalTaskList(data.get("messageKey").toString());
                        break;
                    case SESSION_QUERY_LOCAL_TASK_DETAIL:// 需要查询本地已经采集完毕的指定任务结果
                        TaskService.uploadLocalTaskDetail(data.get("messageKey").toString() , data.get("taskFingerprint").toString());
                        break;
                    default:
                }
            }
